...
def main():
    ...
    parser.add_argument("-p", "--pop_size",
                        required=False, nargs='?')
    # search space
    parser.add_argument("-x", "--pipeline_space",
                        required=False, nargs='?')
    ...
    search_space = args.pipeline_space
    ...
    try:
        ...
        if search_space == "constrained":
            search_space = get_pipeline_space(seed=run_num)
        ...
        # Base tpot run
        est = tpot2.TPOTEstimator(search_space=search_space, generations=gens, population_size=pop_size, cv=5, n_jobs=n_jobs, max_time_mins=None, max_eval_time_mins=5,
