    ...
    try:
        ...
        for task_id in task_ids:
            for i in range(num_runs):
                ...
                constrained_search_space = get_pipeline_space(seed=i)
                ...
                est_random = tpot.TPOTEstimator(search_space=constrained_search_space, generations=0, population_size=100, cv=5, max_time_mins=None,
                ...
                #reference_model_pred = get_cv_probas(reference_model, X_train, y_train, cv_splits=5, random_state=i)

                reference_model = clone(reference_model).fit(X_train, y_train)

                # tpot run with diversity objective, then ES
                ...
                est = tpot.TPOTEstimator(search_space=constrained_search_space, generations=5, population_size=10, cv=5, max_time_mins=None,
                                        random_state=i, verbose=2, classification=True, scorers=['roc_auc_ovr', diversity], scorers_weights=[1, 1])
