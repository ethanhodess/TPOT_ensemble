...
def main():
    ...
    # diversity second objective
    def make_diversity_objective(reference_model):
        def diversity_objective(estimator, X, y=None):
            pred = estimator.predict_proba(X)
            reference_pred = reference_model.predict_proba(X)
            return np.mean(np.abs(pred - reference_pred))
    ...
    try:
        ...
        # first, random search to find reference model
        est_random = tpot2.TPOTEstimator(search_space=constrained_search_space, generations=0, population_size=2000, cv=5, n_jobs=n_jobs, max_time_mins=None,
        ...
        reference_eval_inds = clean_eval_inds(est_random.evaluated_individuals)
        reference_model_idx = reference_eval_inds["roc_auc_score"].idxmax()
        reference_model = reference_eval_inds.loc[reference_model_idx].iloc[10]

        reference_model = clone(reference_model).fit(X_train, y_train)


        # tpot2 run (50x40) and ES
        diversity = make_diversity_objective(reference_model)
        est = tpot2.TPOTEstimator(search_space=constrained_search_space, generations=50, population_size=40, cv=5, n_jobs=n_jobs, max_time_mins=None,
                                 random_state=run_num, verbose=2, classification=True, scorers=['roc_auc_ovr', diversity], scorers_weights=[1, 1])
        ...
        # ensemble selection
        # ensemble_2000 = greedy_forward_search(filtered_eval_inds, X_train, y_train, run_num)
        ensemble_100 = greedy_forward_search(top100, X_train, y_train, run_num)

        # get probas and convert to auroc score
        # ensemble_test_proba_2000 = vote_soft_proba(estimators=ensemble_2000, X_test=X_test)

        # if len(np.unique(y_test)) == 2:
        #     ensemble_test_auroc_2000 = roc_auc_score(
        #         y_test,
        #         ensemble_test_proba_2000[:, 1]
        #     )
        # else:
        #     ensemble_test_auroc_2000 = roc_auc_score(
        #         y_test,
        #         ensemble_test_proba_2000,
        #         multi_class="ovr",
        #         average="macro"
        #     )

        ensemble_test_proba_100 = vote_soft_proba(estimators=ensemble_100, X_test=X_test)

        if len(np.unique(y_test)) == 2:
            ensemble_test_auroc_100 = roc_auc_score(
                y_test,
                ensemble_test_proba_100[:, 1]
            )
        else:
            ensemble_test_auroc_100 = roc_auc_score(
                y_test,
                ensemble_test_proba_100,
                multi_class="ovr",
                average="macro"
            )
        ...
        full_results.append({"task id": task_id,
                            ...
                            # "ensemble_2000": ensemble_test_auroc_2000,
                            "ensemble_100": ensemble_test_auroc_100,
                            })
